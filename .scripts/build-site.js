...
const PATH_JAVASCRIPT = path.join(__dirname, '../javascript/');
...
generate(PATH_JAVASCRIPT);
generate(PATH_PLAYGROUND);

function generate(pathPlayground) {
  const pathPlaygroundReadme = path.join(pathPlayground, 'README.md');
  let tIndex;
...
// 读取 playground 文件列表
  _p = _p.then(() => {
    ...
    return pReadDir(pathPlayground);
  });
...
// 读取 playground 以下的所有内容状态
  _p = _p.then(files => {
    ...
    return Promise.all(files.map(file => {
      const _path = path.join(pathPlayground, file);
...
// 判断每个目录下是否有 README.md 文件
  _p = _p.then(stats => {
    ...
    return Promise.all(stats.map(stat => {
      if (stat.path === pathPlaygroundReadme) {
        return {
